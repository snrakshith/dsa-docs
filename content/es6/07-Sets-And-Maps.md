...
To alleviate such issues, ECMAScript 6 also includes _weak sets_, which only store weak object references and cannot store primitive values. A _weak reference_ to an object does not prevent garbage collection if it is the only remaining reference.
