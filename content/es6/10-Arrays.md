---
title: "10 Improved Array Capabilities"
description: ""
---
...
Continuing the trend from ECMAScript 5, ECMAScript 6 adds several new methods to arrays. The `find()` and `findIndex()` methods are meant to aid developers using arrays with any values, while `fill()` and `copyWithin()` are inspired by use cases for _typed arrays_, a form of array introduced in ECMAScript 6 that uses only numbers.
...
The float-related methods are only different in that they accept an additional optional boolean indicating whether the value should be read or written as little-endian. (_Little-endian_ means the least significant byte is at byte 0, instead of in the last byte.)
